...
function build_lr-duckstation() {
    ...
    if isPlatform "armv8"; then
        params+=(-DCMAKE_C_FLAGS="-march=armv8-a+crc+simd -mtune=cortex-a72")
        params+=(-DCMAKE_CXX_FLAGS="-march=armv8-a+crc+simd -mtune=cortex-a72")
    fi
...
function configure_lr-duckstation() {
    ...
    iniSet "duckstation_GPU.Renderer" "vulkan"
