...
rp_module_id="amiberry-lepotato"
...
# Changed git:// protocol to https:// to avoid authentication
rp_module_repo="git https://github.com/BlitterStudio/amiberry.git :_get_branch_amiberry"
...
function _get_branch_amiberry() {
    if isPlatform "dispmanx"; then
        ...
        echo "preview-v6.3.3"
    elif isPlatform "aarch64" && isPlatform "rpi"; then
        echo "v5.7.2"
    else
        echo "v5.7.2"
    fi
...
function depends_amiberry() {
    ...
    isPlatform "aarch64" && depends+=(libvdpau-dev)

    getDepends "${depends[@]}"
}

function sources_amiberry() {
    # Add depth=1 to make cloning faster and avoid credential prompts for history
...
function configure_amiberry() {
    addEmulator 1 "amiberry-lepotato" "amiga" "$md_inst/amiberry.sh %ROM%"
    addEmulator 0 "amiberry-lepotato-a500" "amiga" "$md_inst/amiberry.sh %ROM% --model A500"
    addEmulator 0 "amiberry-lepotato-a500plus" "amiga" "$md_inst/amiberry.sh %ROM% --model A500P"
    addEmulator 0 "amiberry-lepotato-a1200" "amiga" "$md_inst/amiberry.sh %ROM% --model A1200"
    addEmulator 0 "amiberry-lepotato-a4000" "amiga" "$md_inst/amiberry.sh %ROM% --model A4000"
    addEmulator 0 "amiberry-lepotato-cdtv" "amiga" "$md_inst/amiberry.sh %ROM% --model CDTV"
    addEmulator 0 "amiberry-lepotato-cd32" "amiga" "$md_inst/amiberry.sh %ROM% --model CD32"
    addSystem "amiga"
